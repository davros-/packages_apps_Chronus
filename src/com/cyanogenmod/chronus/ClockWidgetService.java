...
import com.cyanogenmod.chronus.misc.CalendarInfo;
import com.cyanogenmod.chronus.misc.CalendarInfo.EventInfo;
...
public class ClockWidgetService extends IntentService {
    ...
    public static final String ACTION_REFRESH = "com.cyanogenmod.chronus.action.REFRESH_WIDGET";
    public static final String ACTION_REFRESH_CALENDAR = "com.cyanogenmod.chronus.action.REFRESH_CALENDAR";
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null && ACTION_REFRESH_CALENDAR.equals(intent.getAction())) {
            if (DEBUG) Log.d(TAG, "Forcing a calendar refresh");
    ...
    private boolean refreshCalendar(RemoteViews calendarViews) {
        ...
        for (EventInfo event : mCalendarInfo.getEvents()) {
            ...
            if (DEBUG)
                Log.d(TAG, "Showing event: " + event.title);
        ...
        try {
            ...
            if (cursor != null) {
                ...
                for (int i = 0, eventCount = 0; i < cursor.getCount() && eventCount < Constants.MAX_CALENDAR_ITEMS; i++) {
                    ...
                    if (DEBUG)
                        Log.d(TAG, "Adding event: " + title + " with id: " + eventId);
